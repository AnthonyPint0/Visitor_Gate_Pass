...
router.get("/search-purpose", async (req, res) => {
  try {
...
router.get("/search-available-cards", async (req, res) => {
  try {
    const query = req.query.query || "";
    const regex = new RegExp(query, "i"); // Case-insensitive regex for matching purposes
...
router.get("/verify-id-availability", async (req, res) => {
  try {
...
router.get("/verify-visitor-accessibility", async (req, res) => {
  try {
...
router.post("/process-visitor-checkout", async (req, res) => {
